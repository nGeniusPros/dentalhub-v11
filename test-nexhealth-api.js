...
async function testNexHealth() {
  try {
    ...
    if (patientsResponse.data?.data && patientsResponse.data.data.length > 0) {
      ...
      // Save the first 5 patients to a file for reference
      const fs = require('fs');
      const samplePatients = patientsResponse.data.data.slice(0, 5);
      fs.writeFileSync('nexhealth-patients-sample.json', JSON.stringify(samplePatients, null, 2));
      console.log('\nSaved first 5 patients to nexhealth-patients-sample.json');
    } else {

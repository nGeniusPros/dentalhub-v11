...
const PatientLogin = () => {
  ...
  // Use the MCP hook for the patient login request
  const {
    execute: login,
    loading: apiLoading,
    error: apiError
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await login({
      ...
      role: 'patient' // Specify role for the backend adapter
    });
    ...
    if (result === null) {
      ...
      // Optionally set formError if specific handling is needed beyond displaying apiError
      // setFormError(apiError instanceof Error ? apiError.message : 'Login failed.');
      return;
    }

    // On successful login via MCP, navigation might be handled by AuthProvider
    // listening to session changes, or redirect manually if needed.
    // The original comment mentioned ProtectedRoute handling navigation.
    // If manual redirect is needed: window.location.href = '/patient-dashboard';
  };
  ...
  // Note: The original code had a separate 'loading' state, now replaced by 'apiLoading'
  // We can add a specific message for the API loading state if desired.
  if (apiLoading) {
     return (
       <div className="min-h-screen flex items-center justify-center">
         <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-turquoise" />
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-br from-turquoise/5 via-white to-gray-lighter">
      <motion.div
      ...
      >
        ...
        <div className="w-full md:w-1/2 p-8">
          ...
          <LoginNavigation currentPage="patient" showForgotPassword={true} />
        </div>
      </motion.div>
    </div>
  );
